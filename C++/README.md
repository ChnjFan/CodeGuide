...
- [结构化绑定](./Basic/structured_binding.md) - 用一行代码把数组、结构体、pair/tuple 等符合类型成员解包到独立变量中。

## 内存管理

- [内存模型](./Basic/memory/memory.md) - C/C++ 内存模型包含栈、堆、未初始化数据段、数据段和代码段。
- [内存池](./Basic/memory/memory_pool.md) - 多层级内存块管理，智能碎片整理，线程安全机制和高性能对象池。
- [智能指针](./Basic/memory/auto_ptr.md) - 封装裸指针的类模板，利用析构函数自动调用特性释放内存。
...
- [迭代器](./STL/iterator.md) - 为 STL 提供了统一的方式遍历不同容器的元素，行为类似指针。
