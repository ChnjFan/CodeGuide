...
- **距离**：默认以跳数为单位，每经过一个网络 / 网关计 1 跳，范围 1-15（16 表示 “不可达”，视为无穷大）。
...
节点启动时，仅配置**直接连接的网络**到路由表中。

直连网络的路由条目：目的地址为该网络地址，距离为该网络的配置代价（默认为 1），下一跳为空，初始化超时定时器。
...
超时定时器：路由连续 180 秒未收到更新标记为无效（度量为 16）。

垃圾回收定时器：无效路由保留 120 秒，改路由仍向邻居广播不可达，避免邻居仍使用旧路由。
...
水平分割是 RIP 协议优化路由循环问题的机制，核心思路是**避免节点向路由的学习来源反向传播该路由**，防止双向循环。
