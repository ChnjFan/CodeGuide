...
- 防止旧的重复连接成功建链：由于网络延迟或者路由震荡的原因产生。
  - 客户端发起连接请求 SYN 报文但没到服务端
  - 重传定时器超时重新发送 SYN 报文
  - 新建连接断连之后服务端收到了之前的 SYN 报文
- 两次握手的问题：
  - 客户端只发一次 SYN 超时重发
  - 服务端收到 SYN 就完成建链，回复 SYN+ACK
    - 分配资源给连接造成浪费
    - 旧连接数据报文到达造成数据错乱

通过客户端的最后一次确认，过滤到无效的请求连接。
...
⚠️注意：已发送但未收到确认的数据也在发送窗口内，所以，TCP 实际发送新数据的包数=发送窗口大小 - 未确认数据包。
...
接收方接收到数据后不立即回复 ACK，而是等一小段时间后回复累计 ACK，减少网络中单独 ACK 报文的数量降低带宽消耗。

- 延时 ACK 可能会影响发送方的超时重传机制（若 ACK 延迟过久，发送方可能误判丢包并重传），因此超时时间通常严格限制在 200ms 内。
- 对于连续接收多个数据包的场景（如批量数据传输），接收方不会无限等待，通常每接收 2 个数据包就会发送一个 ACK，避免发送方等待过久。
...
拥塞避免采用 “线性增长” 策略，**每个 RTT 周期增加 1 个 MSS**，更平缓地试探网络承载能力。

> 1. 慢启动和拥塞避免阶段最大的区别就是，新的 ACK 到达时，cwnd 怎么增长。
> 2. 没有丢包情况下，ssthresh 使用上次最优值；发生重传后调整为 cwnd 的 1/2。

#### 快速恢复

发送方收到 3 个重复 ACK 后将快速重传丢失的数据包，在检测到数据包丢失但网络未完全拥塞时快速恢复传输效率，避免因频繁进入慢启动阶段导致的性能下降。

> **为什么收到 3 个重复 ACK 需要快速恢复？**
>
> 1. 中间的某个数据包丢失，接收方会持续发送确认号为丢失数据序列号的 ACK，直到收到丢失的包。
> 2. 接收方收到重复的 ACK，TCP 判定某个数据丢失，能收到重复的 ACK 说明网络仍可以传输而非完全拥塞。

**实现算法**：

- **Reno**：基础实现，恢复后进入拥塞避免。
- **NewReno**：优化多包丢失场景，若在快速恢复阶段发现多个包丢失，会持续重传并调整 cwnd，避免多次进入恢复流程。
- **CUBIC**：保留快速恢复的核心逻辑，但 cwnd 恢复时采用三次函数增长（而非线性），更适应高带宽网络。

#### 转发确认

转发确认（FACK）在网络出现丢包时，更精准地识别丢包范围、减少无效重传，并高效调整拥塞窗口，从而提升 TCP 连接的吞吐量和稳定性。

- **精确丢包范围**：FACK 根据 SACK 提供的非连续接收数据块信息计算未确认的数据范围，仅重传真正丢失的数据，避免过度重传浪费带宽。
- **优化拥塞窗口调整策略**：
  - 计算丢失数据字节数和未确认字节数比例。
  - 拥塞窗口缩减量为 max(丢失字节数, cwnd/2)。
- **加速重传和恢复过程**：
  - 根据 SACK 信息收集到丢失范围后立即重传，不用等到超时重传或 3 次重复 ACK。
  - SACK 确认部分数据后，发送方将这部分数据从未确认队列中移除，释放拥塞窗口用于发送新数据，避免链路空闲。

#### 拥塞窗口校验

塞窗口校验机制（CWV）确保在长时间暂停发送数据后，重新校准 cwnd 和 ssthresh，确保与当前网络状态匹配。

发送数据时距离上次发送操作超过一个 RTO：

- **调整慢启动阈值（ssthresh）**：将 ssthresh 更新为 max(ssthresh, (3/4) × 当前cwnd)
- **调整拥塞窗口（cwnd）**：
  - 短时间空闲（1 个 RTT 内）：cwnd 调整为 `min(当前cwnd, 上次活跃时的实际使用窗口W_used + 1个SMSS)`，避免突然发送过多数据。
  - 长时间空闲（超过 1 个 RTT）：每经过 1 个 RTT 的空闲时间，cwnd 减半（但不小于 1 个 SMSS，即最大报文段大小），逐步降低窗口至保守值，防止恢复发送时过载网络。

校准之后：

- cwnd 小于 ssthresh，进入**慢启动阶段**。
- cwnd 大于等于 ssthresh，进入**拥塞避免阶段**。
