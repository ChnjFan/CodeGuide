...
一个接口可以有多个不同类型或不同范围的 IPv6 地址，包括单播地址、任播地址还可组播地址。

**单播地址**

- 链路本地地址：前缀为 fe80::/10，用于统一网络下设备的邻居发现、自动地址配置等，该地址数据不会转发到其他链路。
- 全球单播地址：前缀为 2000::/3，用于与外部网络通信。
- 本地单播地址：私有网络内部通信。
...
IP 数据报在传输中会经过各种网络链路，每条链路都有最大传输单元 MTU 的限制，当数据报大小超过 MTU 后，就需要将其分割成多个较小的片段进行传输，直到数据分片到达最终目的地才会被重组。
